	...
	$scope.findOne=function(){	
		seckillGoodsService.findOne($location.search()['id']).success(
			function(response){
				...
				// console.log(new Date().setDate((new Date().getHours()-1)));
				// console.log(new Date().getTime());
				// second =Math.floor( ( new
				// Date($scope.entity.endTime).getTime()- (new
				// Date().getTime())) /1000); // 总秒数
				second =Math.floor( ((new Date().getTime()) - new Date().setDate((new Date().getHours()-1))) /1000); // 总秒数
				// console.log(second);
